...
import { IBook, Messages } from '@/app/types';
...
function getErrorMessage(error: unknown) {
    ...
    if (error && typeof error === 'object') {
        const details = error as { message?: unknown; error?: unknown; reason?: unknown };
        return String(details.message || details.error || details.reason || JSON.stringify(error));
    }
    return '';
}
...
function getStartErrorMessage(error: unknown) {
    ...
    if (errorMessage.includes('microphone') || errorMessage.includes('permission')) {
        return 'Microphone permission is required. Allow microphone access and try again.';
    }

    if (errorMessage.includes('network') || errorMessage.includes('connection')) {
        return 'Could not connect to Vapi. Please check your internet and try again.';
    }

    if (errorMessage.includes('assistant') || errorMessage.includes('api key') || errorMessage.includes('token')) {
        return 'Vapi assistant setup is invalid. Check your Vapi public key and assistant ID.';
    }

    if (errorMessage.includes('voice') || errorMessage.includes('provider')) {
        return 'Voice setup failed. Please choose another voice or check your Vapi assistant settings.';
    }
...
export function useVapi(book: IBook) {
    ...
    useEffect(() => {
        ...
        const handlers = {
            ...
            error: (error: unknown) => {
                ...
                const errorMessage = getErrorMessage(error).toLowerCase();
                if (errorMessage.includes('microphone') || errorMessage.includes('permission')) {
                    setLimitError('Microphone permission is required. Allow microphone access and try again.');
                ...
                } else if (errorMessage.includes('voice') || errorMessage.includes('provider')) {
                    setLimitError('Voice setup failed. Please choose another voice or check your Vapi assistant settings.');
                } else {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            // The actual limit is enforced by the latest subscription limits from the client hook.
            ...
            if (sessionIdRef.current) {
                await endVoiceSession(sessionIdRef.current, durationRef.current).catch((sessionErr) =>
                    console.error('Failed to clean up voice session after start error:', sessionErr),
                );
                sessionIdRef.current = null;
            }

            setStatus('idle');
            setLimitError(getStartErrorMessage(err));
        }
    }, [book._id, book.title, book.author, durationRef, maxDurationSeconds, voice, userId]);
    ...
    const stop = useCallback(async () => {
        ...
        try {
            ...
            setStatus('idle');
            setLimitError('Failed to stop the voice session. Please refresh and try again.');
        }
    }, []);
